...
<template>
  <header class="flex relative h-10">
    <div v-if="route.path !== '/'" class="absolute top-0 left-0 h-full w-10">
      <button class="flex justify-center h-full w-full items-center bg-blue-300 font-bold text-xl"
  ...
  <main>
    <nav v-if="route.path === '/'" class="flex flex-col gap-1">
      <span v-for="({title, isShowHome}, path) in TEXT_PATH_MAP" class="text-blue-300 text-center text-2xl underline underline-offset-8">
